...
export default function SeatSelectionPage() {
  ...
  const [selectedSection, setSelectedSection] = useState("Section-1"); // Default section
  const [selectedSeats, setSelectedSeats] = useState([]); // User's selected seats
  const [loading, setLoading] = useState(true); // Loading state
  const [error, setError] = useState(null); // Error state
  const [token, setToken] = useState(null); // User authentication token
